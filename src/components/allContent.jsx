...
//   // Progress bar logic: ONLY MODULES
//   const totalModules = modules.length;
//   const completedModules = modules.filter(
//     m => progress[m._id]?.completed
//   ).length;
...
//   // === YouTube Player Handlers (unchanged) ===
//   function onPlayerReady(event) {
//     try {
//       if (user?.role !== "student") return;
//       playerRef.current = event.target;
...
//     }
//   }
//   // === End of YouTube Player Handlers ===
...
//                   }
//                   tabIndex={0}
//                   onClick={() => setSelectedSidebar("quiz")}
//                 >
//                   <span className="allcontent-module-title">Quiz</span>
//                   {/* No status, no icons */}
//                 </div>
...
//                 <div
//                   className="allcontent-desc-body"
//                   dangerouslySetInnerHTML={{ __html: stripYoutubeEmbeds(selectedModule.description) }}
//                 />
//                 {selectedModule && Array.isArray(selectedModule.files) && selectedModule.files.filter(f => f.type !== "video").length > 0 && (
//                   <div className="allcontent-files-section">
//                     <div className="allcontent-files-label">
//                       Attached Files ({selectedModule.files.filter(f => f.type !== "video").length.toString().padStart(2, "0")})
//                     </div>
//                     <div className="allcontent-files-list">
//                       {selectedModule.files.filter(f => f.type !== "video").map((file, i) => {
//                         const isImage = /\.(jpe?g|png|gif|bmp|webp)$/i.test(file.name);
//                         const hasKey = !!file.key;
//                         const downloadUrl = hasKey
//                           ? `${window.location.hostname === "localhost"
